const APP = {
    ...
    allOrders: [], filteredOrders: [], currentFilter: 'all', currentSearch: '',
    shopName: 'kedai kami', shopUrl: '', currentPage: 1, itemsPerPage: 10,

    // KUNCI KESELAMATAN ADMIN
    getAdminToken: function() { 
        return localStorage.getItem("admin_secret_token") || ""; 
    },

    // 1. TAMBAH FUNCTION TOKEN VALIDITY
    checkTokenValidity: function(res) {
        if (res.status === 'error' && (res.msg?.includes('token') || res.msg?.includes('unauthorized') || res.msg?.includes('Tidak dibenarkan') || res.msg?.includes('Admin'))) {
            localStorage.removeItem('admin_secret_token');
            Swal.fire({
                icon: 'error',
                title: 'Sesi Tamat',
                text: 'Sila login semula untuk meneruskan.',
                confirmButtonText: 'Ke Halaman Login',
                confirmButtonColor: '#3b82f6',
                allowOutsideClick: false
            }).then(() => {
                // Bergantung pada susun atur Tuan, ini akan reload ke halaman admin utama
    ...
    startLoad: async function() {
        ...
        try {
            ...
            this.updateStats(); this.processData(); this.syncUnpaidOrders();
        } catch(e) { 
    ...
    silentRefresh: async function() {
        try {
            ...
            this.allOrders = resOrders.orders || (Array.isArray(resOrders) ? resOrders : []);
            this.updateStats(); this.processData(); 
    ...
    factoryReset: async function() {
        const { value: text } = await Swal.fire({ title: 'PENGESAHAN AKHIR', html: 'Sila taip perkataan <b>RESET</b> di bawah untuk meneruskan pemadaman:', input: 'text', inputPlaceholder: 'Taip RESET di sini', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#6b7280', confirmButtonText: 'Padam Sekarang', cancelButtonText: 'Batal' });
        if (text === 'RESET') {
            ...
            try {
                const res = await fetch(this.workerURL, { 
                    ...
                    body: JSON.stringify({ action: 'factory_reset', admin_token: this.getAdminToken() }) 
                }).then(r => r.json());
                
                // 2. INTEGRATE TOKEN CHECK
                if (!this.checkTokenValidity(res)) return;
                ...
                Swal.fire('Ralat Rangkaian', 'Gagal berhubung dengan server. Sila periksa internet anda.', 'error'); 
            }
        } else if (text !== undefined) { Swal.fire('Dibatalkan', 'Perkataan tidak sepadan. Pemadaman dibatalkan demi keselamatan.', 'info'); }
    },
    ...
    search: function(val) {
        ...
        this.currentPage = 1; this.processData();
    },
    ...
    filter: function(status, el) { 
        ...
        this.currentFilter = status; this.currentPage = 1; this.processData();
    },
    ...
    processData: function() {
        ...
        else if (this.currentPage > totalPages) { this.currentPage = totalPages; } 
        else if (this.currentPage < 1) { this.currentPage = 1; }

        this.updatePaginationUI(); this.updateStatusText(false);
    },
    ...
    updatePaginationUI: function() {
        ...
        if (totalItems <= this.itemsPerPage) {
            ...
            return;
        } else { if (paginationEl) paginationEl.classList.remove('hidden'); }
    ...
    prevPage: function() { if (this.currentPage > 1) { this.currentPage--; this.updatePaginationUI(); window.scrollTo({ top: 0, behavior: 'smooth' }); } },
    nextPage: function() { const totalPages = Math.ceil(this.filteredOrders.length / this.itemsPerPage); if (this.currentPage < totalPages) { this.currentPage++; this.updatePaginationUI(); window.scrollTo({ top: 0, behavior: 'smooth' }); } },
    highlightText: function(text, search) { if (!search || !text) return text; const regex = new RegExp(`(${search.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'gi'); return text.replace(regex, '<span class="search-highlight">$1</span>'); },
    ...
    render: function(orders) {
        const list = document.getElementById('order-list');
        if(orders.length === 0) { list.innerHTML = `<div class="text-center py-20 bg-white rounded-3xl border border-slate-200"><div class="w-16 h-16 mx-auto mb-4 bg-slate-50 rounded-full flex items-center justify-center text-slate-300"><i class="ri-inbox-2-line text-3xl"></i></div><p class="text-slate-500 font-medium">Tiada rekod pesanan dijumpai.</p></div>`; return; }
        
        let html = '';
        orders.forEach(o => {
            let badgeClass = 'badge-unpaid'; let statusText = o.status;
            ...
            let displayName = o.customer_name || 'Pelanggan'; let displayId = `#${(o.id || '').slice(-6)}`; let displayPhone = o.customer_phone || '';
            
            if (this.currentSearch) { displayName = this.highlightText(displayName, this.currentSearch); displayId = this.highlightText(displayId, this.currentSearch); displayPhone = this.highlightText(displayPhone, this.currentSearch); }
            ...
            <div class="order-card">
                ...
                <div class="shrink-0 flex items-center gap-2 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6 justify-end">
                    ${o.status !== 'DELETED' ? `<button onclick="APP.deleteOrder('${o.id}')" class="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-rose-500 bg-slate-50 hover:bg-rose-50 rounded-xl transition" title="Padam Order"><i class="ri-delete-bin-line text-lg"></i></button>` : `<div class="w-10 h-10 flex items-center justify-center text-rose-300 bg-rose-50 rounded-xl"><i class="ri-delete-bin-2-fill text-lg"></i></div>`}
    ...
    copyText: function(btn) {
        const text = btn.getAttribute('data-copy'); if (!text) return;
        navigator.clipboard.writeText(text).then(() => {
            ...
            btn.classList.replace('text-slate-500', 'text-emerald-600'); btn.classList.replace('hover:text-blue-600', 'hover:text-emerald-600'); btn.classList.replace('bg-slate-50', 'bg-emerald-50'); btn.classList.add('border', 'border-emerald-200', 'w-auto', 'px-3');
            setTimeout(() => { btn.innerHTML = originalHTML; btn.classList.replace('text-emerald-600', 'text-slate-500'); btn.classList.replace('hover:text-emerald-600', 'hover:text-blue-600'); btn.classList.replace('bg-emerald-50', 'bg-slate-50'); btn.classList.remove('border', 'border-emerald-200', 'w-auto', 'px-3'); }, 1500);
        }).catch(() => { Swal.fire({ toast: true, position: 'top-end', icon: 'error', title: 'Gagal salin teks!', showConfirmButton: false, timer: 1500 }); });
    },

    showDetail: function(id) {
        const order = this.allOrders.find(o => String(o.id) === String(id)); if(!order) return;
        ...
        let items = []; try { if (order.items) items = typeof order.items === 'string' ? JSON.parse(order.items) : order.items; if (typeof items === 'string') items = JSON.parse(items); } catch(e){}
        ...
        const createCopyBlock = (label, value, isMultiline = false) => {
            if(!value || value === '-') return ''; const safeAttrValue = value.replace(/&/g, '&amp;').replace(/"/g, '&quot;');
            return `
                <div class="flex items-start justify-between gap-3 p-3.5 bg-white rounded-xl border border-slate-200 mb-3 shadow-sm hover:border-blue-200 transition-colors">
                    <div class="flex-1 min-w-0"><div class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">${label}</div><div class="text-sm font-bold text-slate-800 ${isMultiline ? 'leading-relaxed whitespace-pre-line' : 'truncate'}">${value}</div></div>
                    <button onclick="APP.copyText(this)" data-copy="${safeAttrValue}" class="shrink-0 flex items-center justify-center w-9 h-9 bg-slate-50 hover:bg-blue-50 text-slate-500 hover:text-blue-600 rounded-lg transition" title="Copy ${label}"><i class="ri-file-copy-line text-lg"></i></button>
                </div>`;
        };

        const custName = order.customer_name || 'N/A'; const custPhone = order.customer_phone || ''; const custEmail = order.customer_email || ''; const custAddress = order.ship_address || '';
        const bodyHTML = `
            <div class="space-y-6">
                ${reasonHtml}
                <div>
                    ...
                    <div class="bg-slate-100 p-3 rounded-2xl border border-slate-200">
                        ${createCopyBlock('Nama Penuh', custName)} ${createCopyBlock('No. Telefon', custPhone)} ${createCopyBlock('E-mel', custEmail)} ${createCopyBlock('Alamat Penghantaran', custAddress, true)}
                    </div>
                </div>
                <div>
        ...
        if(order.status === 'DELETED') { 
            ...
            footerHtml = `<div class="w-full sm:w-1/3 text-center text-emerald-600 font-bold text-sm bg-emerald-50 p-3.5 rounded-xl border border-emerald-100 flex items-center justify-center gap-2"><i class="ri-checkbox-circle-fill text-lg"></i> Selesai</div><button onclick="APP.whatsappCustomer('${order.id}')" class="flex-1 bg-[#25D366] text-white py-3.5 rounded-xl font-bold text-sm hover:bg-[#128C7E] transition shadow-lg shadow-green-200 flex items-center justify-center gap-2"><i class="ri-whatsapp-fill text-xl"></i> WhatsApp Pelanggan</button>`;
        } else if (order.status === 'UNPAID') {
            footerHtml = `<button onclick="APP.checkPayment('${order.payment_ref}')" class="flex-1 bg-amber-500 text-white py-3.5 rounded-xl font-bold text-sm hover:bg-amber-600 transition shadow-lg shadow-amber-200 flex items-center justify-center gap-2"><i class="ri-refresh-line text-lg"></i> Semak Bayaran</button><button onclick="APP.markPaidManual('${order.id}')" class="flex-1 bg-white border-2 border-slate-200 text-slate-700 py-3.5 rounded-xl font-bold text-sm hover:bg-slate-50 transition flex items-center justify-center gap-2"><i class="ri-check-double-line text-lg"></i> Force Paid</button>`;
        } else {
            if(order.tracking_number) { 
                footerHtml = `<button onclick="APP.sendEmailAndComplete('${order.id}')" class="w-full bg-emerald-600 text-white py-4 rounded-xl font-bold text-sm hover:bg-emerald-700 transition shadow-lg shadow-emerald-200 flex items-center justify-center gap-2 uppercase tracking-wide"><i class="ri-mail-send-fill text-lg"></i> Email Resit & Selesai</button>`; 
            } else { 
        ...
        document.getElementById('admin-modal-footer').innerHTML = footerHtml; document.getElementById('admin-specific-modal').classList.add('active');
    },

    closeModal: function() { document.getElementById('admin-specific-modal').classList.remove('active'); },

    whatsappCustomer: function(id) {
        const order = this.allOrders.find(o => String(o.id) === String(id)); if (!order) return;
        let phone = order.customer_phone || ''; phone = phone.replace(/\D/g, ''); if (phone.startsWith('0')) phone = '6' + phone;
    ...
    inputTracking: async function(id, currentVal = '') {
        ...
        if (trackingNo) {
            ...
            try {
                const res = await fetch(this.workerURL, { 
                    method: 'POST', 
                    body: JSON.stringify({ action: 'save_tracking_manual', order_id: id, tracking_number: trackingNo, admin_token: this.getAdminToken() }), 
                    headers:{'Content-Type':'application/json'} 
                }).then(r=>r.json());
                ...
                if(res.status === 'success') { 
                    ...
                    this.closeModal(); this.silentRefresh(); 
                } else Swal.fire('Ralat', res.msg, 'error');
            } catch(e) { 
                // 3. IMPROVED ERROR HANDLING
                Swal.fire('Ralat Rangkaian', 'Gagal berhubung dengan server. Sila periksa internet anda.', 'error'); 
    ...
    markPaidManual: async function(id) {
        ...
        if (result.isConfirmed) {
            ...
            try {
                const res = await fetch(this.workerURL, { 
                    method: 'POST', 
                    body: JSON.stringify({action:'manual_mark_paid', order_id:id, admin_token: this.getAdminToken()}), 
                    headers:{'Content-Type':'application/json'} 
                }).then(r=>r.json());
                ...
                // 3. IMPROVED ERROR HANDLING
                Swal.fire('Ralat Rangkaian', 'Gagal memproses permintaan. Sila periksa internet anda.', 'error'); 
            }
        }
    },
    ...
    sendEmailAndComplete: async function(id) {
        const order = this.allOrders.find(o => String(o.id) === String(id)); if (!order) return;
        let items = []; try { items = JSON.parse(order.items || '[]'); } catch(e) {}
        let itemText = ""; let subtotal = 0; let itemNamesArray = [];
        items.forEach((item, index) => { let itemTotal = parseFloat(item.price) * parseInt(item.qty); subtotal += itemTotal; itemText += `${index + 1}. ${item.name} (x${item.qty}) - RM${itemTotal.toFixed(2)}\n`; itemNamesArray.push(item.name); });
        let itemNames = itemNamesArray.join(', '); const reviewUrl = this.shopUrl; let grandTotal = parseFloat(order.total); let postage = Math.max(0, grandTotal - subtotal);
        ...
        try {
            const res = await fetch(this.workerURL, { 
                method: 'POST', 
                body: JSON.stringify({action:'mark_completed', order_id:id, admin_token: this.getAdminToken()}), 
                headers:{'Content-Type':'application/json'} 
            }).then(r=>r.json());
    ...
    deleteOrder: async function(id) {
        ...
        if (reason) {
            ...
            try {
                const res = await fetch(this.workerURL, { 
                    method: 'POST', 
                    body: JSON.stringify({ action: 'delete_order', id: id, reason: reason, admin_token: this.getAdminToken() }), 
                    headers: { 'Content-Type': 'application/json' } 
                }).then(r=>r.json());
                ...
                // 3. IMPROVED ERROR HANDLING
                Swal.fire('Ralat Rangkaian', 'Gagal memadam order. Sila periksa internet anda.', 'error'); 
            }
        }
    }
};
