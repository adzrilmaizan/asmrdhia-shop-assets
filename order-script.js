const APP = {
  ...
  _pickTime: function (o) {
    const d = o?.date || o?.updated_at || o?.created_at || o?.ts || null;
  ...
  _statusMeta: function (status) {
    ...
    if (st === 'PAID') return { title: 'Pembayaran diterima', icon: 'ri-check-double-line', cls: 'text-emerald-600' };
    if (st === 'UNPAID') return { title: 'Order belum bayar', icon: 'ri-time-line', cls: 'text-amber-600' };
    if (st === 'PROCESSING') return { title: 'Order diproses', icon: 'ri-loader-2-line', cls: 'text-blue-600' };
    if (st === 'COMPLETED') return { title: 'Order selesai', icon: 'ri-award-line', cls: 'text-emerald-600' };
    if (st === 'DELETED') return { title: 'Order masuk sampah', icon: 'ri-delete-bin-6-line', cls: 'text-rose-600' };
    return { title: 'Order dikemaskini', icon: 'ri-edit-2-line', cls: 'text-slate-600' };
  },

  pushActivity: function ({ title, sub, icon, cls } = {}) {
    this.activity.unshift({
      t: new Date(),
      title: title || 'Aktiviti',
      sub: sub || '',
      icon: icon || 'ri-pulse-line',
      cls: cls || 'text-slate-600'
    });
    this.activity = this.activity.slice(0, 30);
    this.renderRecentActivity();
  },

  renderRecentActivity: function (max = 8) {
    const el = document.getElementById('recent-activity');
    if (!el) return;

    const derived = (this.allOrders || [])
      .map(o => {
        const t = this._pickTime(o);
        if (!t) return null;

        const meta = this._statusMeta(o.status);
        const shortId = (o.id || '').toString().slice(-6);
        const name = o.customer_name || 'Pelanggan';
    ...
    const manual = (this.activity || []).slice(0, max);
    ...
    if (!merged.length) {
      el.innerHTML = `<div class="text-sm text-slate-400">Tiada aktiviti lagi.</div>`;
      return;
    }

    el.innerHTML = merged.map(x => `
      <div class="flex items-center gap-3 bg-white/60 border border-slate-100 rounded-2xl px-4 py-3">
        <div class="w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center ${x.cls}">
          <i class="${x.icon} text-lg"></i>
        </div>
        <div class="min-w-0 flex-1">
          <div class="font-black text-slate-800 text-sm truncate">${x.title}</div>
          <div class="text-xs font-medium text-slate-500 truncate">${x.sub || ''}</div>
        </div>
        <div class="text-[11px] font-bold text-slate-400 whitespace-nowrap">${this._fmtTime(x.t)}</div>
      </div>
    `).join('');
  },
  ...
  syncUnpaidOrders: async function () {
    ...
    if (recentUnpaid.length > 0) {
      ...
      if (changes) {
        ...
        this.pushActivity({ title: 'Auto semak bayaran: ada perubahan', sub: 'Status FPX dikemaskini', icon: 'ri-refresh-line', cls: 'text-emerald-600' });
      }
    ...
    if (text === 'RESET') {
      ...
      try {
        ...
        if (res.status === 'success') {
          this.pushActivity({ title: 'Factory reset', sub: 'Semua data dipadam', icon: 'ri-delete-bin-6-line', cls: 'text-rose-600' });
  ...
  showDetail: function (id) {
    ...
    const createCopyBlock = (label, value, isMultiline = false) => {
      ...
      const safeAttrValue = String(value).replace(/&/g, '&amp;').replace(/"/g, '&quot;');
      return `
  ...
  inputTracking: async function (id, currentVal = '') {
    const { value: trackingNo } = await Swal.fire({
      ...
      inputPlaceholder: 'Cth: JNT123456789',
      showCancelButton: true,
    ...
    if (trackingNo) {
      ...
      try {
        const res = await fetch(this.workerURL, {
          method: 'POST',
          body: JSON.stringify({ action: 'save_tracking_manual', order_id: id, tracking_number: trackingNo }),
          headers: { 'Content-Type': 'application/json' }
        }).then(r => r.json());

        if (res.status === 'success') {
          this.pushActivity({ title: 'Tracking dikemaskini', sub: `#${id.toString().slice(-6)} • ${trackingNo}`, icon: 'ri-route-line', cls: 'text-blue-600' });
  ...
  checkPayment: async function (billCode) {
    ...
    try {
      ...
      if (res.paid) {
        this.pushActivity({ title: 'Bayaran FPX disahkan', sub: `Bill: ${billCode}`, icon: 'ri-bank-card-line', cls: 'text-emerald-600' });
  ...
  markPaidManual: async function (id) {
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        if (res.status === 'success') {
          this.pushActivity({ title: 'Force Paid dibuat', sub: `#${id.toString().slice(-6)}`, icon: 'ri-check-double-line', cls: 'text-emerald-600' });
  ...
  sendEmailAndComplete: async function (id) {
    ...
    try {
      ...
      if (res.status === 'success') {
        this.pushActivity({ title: 'Email resit dihantar + order selesai', sub: `#${id.toString().slice(-6)} • ${order.customer_name || ''}`.trim(), icon: 'ri-mail-send-line', cls: 'text-emerald-600' });
  ...
  deleteOrder: async function (id) {
    ...
    if (reason) {
      ...
      try {
        ...
        if (res.status === 'success') {
          this.pushActivity({
            title: 'Order dipadam ke sampah',
            sub: `#${id.toString().slice(-6)} • ${reason.slice(0, 40)}${reason.length > 40 ? '…' : ''}`,
...
// NOTE: guna addEventListener (lebih selamat daripada window.onload)
