const ASMRDHIA_APP = {
    ...
    async request(method, bodyData = null, customAction = null) {
        ...
        if (method === 'GET' && customAction) {
            url += `?action=${customAction}&admin_token=${secret}&_t=${Date.now()}`;
        } else if (method === 'GET') {
            url += `?action=get_menu_data&admin_token=${secret}&_t=${Date.now()}`;
        }
    ...
    renderProductGrid(data = null) {
        ...
        list.forEach(p => {
            ...
            grid.innerHTML += `
                <div class="product-card group ${p.status === 'DRAFT' || meta.isActive === 0 ? 'opacity-70 grayscale-[30%]' : ''}">
    ...
    editProduct(id) {
        ...
        if(statusEl) statusEl.value = (p.status === 'DRAFT' || meta.isActive === 0) ? '0' : '1';
    ...
    async saveProduct() {
        ...
        const finalStatus = document.getElementById('prod-status')?.value || '1';
